...
// 1. Implement a general-purpose promisify function, which takes any function that takes exactly one argument and a callback, and wraps it in a function that returns a promise. Can not run
...
// 2. In the section on Typesafe Protocols we derived one half of a protocol for typesafe matrix math. Given this half of the protocol that runs in the main thread, implement the other half that runs in a Web Worker thread. Can not run
// type Matrix = number[][]
// 
// type MatrixProtocol = {
//   determinant: {
